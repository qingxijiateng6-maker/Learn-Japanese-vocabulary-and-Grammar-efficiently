...
import { getGrammarSessionStaticParamsForLevel } from "@/content/staticParams";
...
import { getGrammarPracticeHref } from "@/domain/grammar/meta";
...
export async function generateStaticParams() {
  return getGrammarSessionStaticParamsForLevel("B1");
}
  ...
  return (
    <PageScaffold className="page-main grammar-page grammar-page--session" backdropVariant="grammar">
      ...
      <section className="page-card grammar-panel grammar-panel--hero grammar-session-hero">
        <p className="flashcard-label">Grammar • B1 • Session {sessionNumber}</p>
        ...
        <div className="button-row">
          {session ? (
            <Link
              ...
              href={getGrammarPracticeHref("B1", parsedSessionNumber)}
            >
              To Practice
            </Link>
          ) : null}
          <Link className="button-link" href="/grammar/b1">
            Back to sessions
          </Link>
        </div>
